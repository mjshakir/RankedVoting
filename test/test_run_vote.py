...
def test_run_vote(ranked_voting):
    ...
    # Verify that the removed candidates are correct
    assert vote_history['removed'].iloc[0] == ['Candidate 2']  # The candidate with the lowest votes in the first round
    assert vote_history['removed'].iloc[1] == ['Candidate 1']  # The remaining candidate with the lowest votes in the second round
